...
\begin{document}
	...
	\begin{titlepage}
		...
		\end{center}
		\begin{center}
			\textsc{\LARGE Faculté des sciences et techniques Mohammedia}\\[0.4cm]
			\textsc{\Large Rapport de stage }\\[0.3cm]
			\begin{center}
				\textsc{	\emph{3éme année FI : Génie Mathématiques et  informatique Année universitaire: 2019/2020}}
				
			\end{center}
			{ \LARGE \bfseries CONCEPTION ET D\'EVELOPPEMENT DE TESTS AUTOMATIQUES (NON R\'EGRESSION, S\'ECURIT\'E ET PERFORMANCE) ET SON INT\'EGRATION DANS LA CHAINE CI/CD DE GitLab \\ }
			\bigskip
			\bigskip
			...
			\begin{minipage}{0.8\textwidth}
				\begin{flushleft} \textbf{\underline{Réalisé par:}
						}\\
					BENAMI Anas [ élève ingénieur ]\\
					\textbf{\underline{Encadré par:}}\\
					M. \hspace{0.1cm}KHACHAFI Elmahdi [à remplir ]\\
					M. \hspace{0.1cm}Khadir Omar[ Professeur à la FST de Mohammedia ]
				\end{flushleft}
			\end{minipage}
			\bigskip
			\begin{flushleft}
	...
	Je tiens à  remercier chaleureusement, mon encadrant  M.  {\bfseries KHACHAFI Elmahdi}[poste à ajouter],  pour le temps qu'il a consacré à m'apporter les outils méthodologiques indispensables à la conduite de ce modeste travail . Son exigence m'a grandement stimulé.\\
	Mes vifs remerciements vont également à M. {\bfseries Khadir Omar}[Professeur à la FST de Mohammedia]  pour sa patience, ses conseils pleins de sens et pour le suivi et l'intérêt qu'il a porté à mon travail. L'ensemble des camarades de classe pour leurs intérêts et leurs encouragements.\\\\
	...
	« Conception et développement de tests automatiques (non régression, sécurité et performance) et son intégration dans la chaine CI/CD [l'intégration continue /livraison et déploiement continue] de GitLab ».
	...
	\begin{itemize}
		...
		\item \underline{Les plateformes : }\\
		\begin{itemize}
			...
			\item \textbf{CasaOpenData :} Portail de données statistiques ouvertes, alimenté à partir des plateformes digitales Casaurba et Rokhas et publiant des indicateurs précis, actualisés au quotidien et conçus en tant qu'outil d'aide à la décision et d'amélioration continue.\\
	...
	$\bullet$ \'Eviter au maximum les erreurs humaines.
	...
	nécessaires pour la réalisation d'un projet quel que soit le secteur 
	...
	Le diagramme suivant va représenter les tâches principales à 
	...
	Pour faciliter la tâche on va utiliser le langage de 
	...
	La deuxième chose, on crée la classe \textbf{BasePage} (C'est une classe Java qui contient comme attributs les éléments web [ Bouton, inputText,  checkBox...  etc ] de la page web qu'on veut tester) on récupère les éléments web à l'aide du framework Selenium.\\\\
	...
	\textbf{Nb} : On peut lancer le nombre de scénarios qu'on veut.  \\
	...
	En cas d'échec d'un test il fait une capture d'écran pour qu'on puisse savoir exactement qu'est ce qui est passé.	
		\begin{center}
			\begin{figure}[!h]
				\centering
				\includegraphics[width=15cm]{images/pfe_nv/rapport_test.png}
	...
	\subsubsection{Les outils de DAST :}
	\textbf{Utilisation :} \\
...
\end{center}
	...
	\begin{center}
		\begin{figure}[!h]
			\centering
			\includegraphics[width=16cm]{images/pfe_nv/homePage_sonarqube.png}
			\caption{HOME page SonareQube }
			\label{page12}
		\end{figure}
	\end{center}
	\textbf{\underline{Mots clés:}}\\
		$\bullet$ \textbf{un bug} est un fonctionnement non désiré dans un programme informatique,On parle de bug quand le logiciel ne fait pas ce qu'on lui a demandé, qu'il se bloque et ne répond plus ou qu'il plante.\\
		$\bullet$ \textbf{vulnérabilités} ce sont les failles de sécurité.\\
		$\bullet$ \textbf{Un point d'accès de sécurité} met en évidence un morceau de code sensible à la sécurité que le développeur doit examiner. Après examen, on constatent qu'il n'y a aucune menace ou bien nous devons appliquer un correctif pour sécuriser le code.\\
		$\bullet$ \textbf{Les mauvaises odeurs} sont des mauvaises pratiques de conception logicielle qui conduisent à l'apparition de défauts.\\\\
	...
	\begin{center}
		\begin{figure}[!h]
			\centering
			\includegraphics[width=18cm]{images/pfe_nv/overview_sonarqube.png}
			\caption{Aperçu SonareQube }
			\label{page13}
		\end{figure}
	\end{center}

	
	On remarque devant chaque élément une lettre par exemple devant Bugs ona E de méme pour vulnérabilités et les points d'accès. mais pour les code smells on a la lettre A.
	
	Les lettres représentent le degré de dangerosité.\\\\
	\textbf{E}: bloqueur ; \textbf{D}: critique ; \textbf{C}: majeur ; \textbf{B}:mineure ; \textbf{A}:pas de problème. \\\\
	...
	\newpage
		\begin{center}
			\begin{figure}[!h]
				\centering
				\includegraphics[width=15cm]{images/pfe_nv/vulnerabilities_sonrqube.png}
				\caption{Problèmes SonareQube }
				\label{page14}
			\end{figure}
		\end{center}
	...
	\begin{center}
		\begin{figure}[!h]
			\centering
			\includegraphics[width=15cm]{images/pfe_nv/sql_injection_sonarqube.png}
			\caption{Les Points d'accès de sécurité SonareQube }
			\label{page15}
		\end{figure}
	\end{center}
	...
	La priorité de révision est déterminée par la catégorie de sécurité de chaque règle de sécurité. Les règles dans les catégories qui sont classées haut sur les standards OWASP Top 10 et CWE Top 25 sont considérées comme ayant une haute priorité d'examen. Les règles dans les catégories qui ne sont pas classées haut ou qui ne sont pas mentionnées dans les standards OWASP Top 10 ou CWE Top 25 sont classées moyenne ou faible.\\\\\\
	 \textbf{\large \underline{Note :}}\\\\
	\textbf{Common Weakness Enumeration [CWE]} est une liste des vulnérabilités que l'on peut rencontrer dans les logiciels. Cette liste est maintenue par l'organisme MITRE, le projet étant soutenu par la National Cyber Security Division et le Département de la Sécurité intérieure des \'Etats-Unis.\\
	\includegraphics[width=4cm]{images/pfe_nv/owasp.png} est une fondation à but non lucratif qui travaille à améliorer la sécurité des logiciels. Grâce à des projets de logiciels open source menés par la communauté, des centaines de sections locales dans le monde, des dizaines de milliers de membres et des conférences éducatives et de formation de premier plan, la Fondation OWASP est la source pour les développeurs et les technologues de sécuriser le Web.\\
	...
	Sonarqube nous donne la possibilité d'exiger nos conditions pour décider est ce que le scan passe ou échoue à travers \textbf{Quality Gates}
	
	   \begin{center}
	   	\begin{figure}[!h]
	   		\centering
	   		\includegraphics[width=13cm]{images/qualityGates.png}
	   		\caption{Quality Gates SonareQube }
	   		\label{page17}
	   	\end{figure}
	   \end{center}
	...
	\begin{center}
		\begin{figure}[!h]
			\centering
			\includegraphics[width=13cm]{images/sonarquberules.png}
			\caption{Rules SonareQube }
			\label{page18}
		\end{figure}
	\end{center} 
	
	\textbf{Deuxième cas: Analyse d'une partie du code source en cours d'exécution de l'entreprise} 
	
		\begin{center}
			\begin{figure}[!h]
				\centering
				\includegraphics[width=17cm]{images/overviewGis.png}
				\caption{Scan du code source de l'entreprise }
				\label{page16}
			\end{figure}
		\end{center}
		Cela peut sembler un petit peu bizarre, mais il y a deux facteurs qui sont responsable de ces chiffres.\\
		$\bullet$ Le premier c'est l'utilisation d'une bibliothèque de XML parse  qui nécessite une configuration représente des problèmes.\\
		$\bullet$ Le deuxième c'est les faux positifs , sonarqube préfère de déclarer q'une ligne dangereuse même si elle n'est pas, que déclarer une ligne dangereuse comme une ligne correcte.   
	\newpage
	...
	$\bullet$ La qualité d'accueil des clients qui se dégrade.\\	
	$\bullet$ La perte de clients.\\
	$\bullet$ La perte de revenu.\\
	
	\subsection{Outils de test :}
	\includegraphics[width=4cm]{images/pfe_nv/JMeter.png} Est un projet de logiciel libre permettant d'effectuer des tests de performance d'applications et de serveurs selon différents protocoles ainsi que des tests fonctionnels. Il est développé au sein de la Fondation Apache (ASF).\\
	...
	\includegraphics[width=4cm]{images/pfe_nv/blazemeter.png}  Commercialise une plateforme commerciale de test de charge en libre-service en tant que service (PaaS), qui est compatible avec Apache JMeter open-source ,dans le cadre de test de performance de la Apache Software Foundation.\\  BlazeMeter a été fondé en 2011 et a été acquis par CA Technologies en 2016.
...
%  $\bullet$ \\
	...
	% \textbf{Note:} le mot clé \underline{tags} a pour but de spécifié le runner qui va exécuté le scripte du stage. Par défaut GitLab contient des runners qui peuvent exécuter les commandes ( Bash, C++, Maven, Nodejs...),Mais dans notre cas, on a ajouté nos 2 runners un pour exécuté les commandes wapiti et l'autre pour exécuté les commandes de Nikto.    
	 ...
	 \section{Exemples d'intégration des tests :}
	 	 Pas Encore ..
